...
fn bindings_info_path() -> Result<PathBuf> {
    const BINDINGS_INFO_FILE: &str = "bindings.wasm";
    Ok(Path::new(env!("CARGO_TARGET_DIR"))
        .join("bindings")
        .join(
            std::env::var("CARGO_CRATE_NAME")
                .expect("failed to get `CARGO_CRATE_NAME` environment variable"),
        )
        .join(BINDINGS_INFO_FILE))
}
...
fn decode_resolve(path: &Path, span: Span) -> Result<(Resolve, WorldId)> {
    let bytes = std::fs::read(path).map_err(|e| {
        Error::new(
            span,
            format!(
                "failed to read bindings information file `{path}`: {e}\n\n\
    ...
    let decoded = wit_component::decode(&bytes)
        .map_err(|e| Error::new(span, format!("failed to decode bindings information: {e}")))?;
    ...
    match decoded {
        DecodedWasm::WitPackage(resolve, pkg) => {
            let world = resolve.select_world(pkg, None).map_err(|e| {
                Error::new(
                    span,
                    format!("failed to select world from bindings information: {e}"),
                )
            })?;
            Ok((resolve, world))
        }
        DecodedWasm::Component(_, _) => Err(Error::new(
            span,
            format!(
                "bindings information file `{path}` is not a WIT package",
                path = path.display()
            ),
        )),
    }
